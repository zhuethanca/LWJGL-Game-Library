...
import com.base.engine.rendering.meshLoading.oobjloader.parser.BuilderInterface;
...
public class Build implements BuilderInterface {
	...
	public IndexedModel toIndexedModel() {
		...
		for (FaceVertex fv : faceVerticeList) {
			Vector3f currentPosition = fv.pos;
			Vector2f currentTexCoord = fv.tex;
			Vector3f currentNormal = fv.normal;
			if (currentTexCoord == null)
		...
		for (Face f : faces)
			for (FaceVertex fv : f.vertices)
				result.getIndices().add(fv.index);
		result.calcNormals();
		return result;
	}
	...
	public void addPoints(int[] values) {
		// log.log(INFO, "@TODO: Got " + values.length + " points in builder,
		// ignoring");
	}

	public void addLine(int[] values) {
		// log.log(INFO, "@TODO: Got a line of " + values.length + " segments in
		// builder, ignoring");
	}
	...
	public void addFace(int[] vertexIndices) {
		...
		while (loopi < vertexIndices.length) {
			...
			if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesG.size())) {
				...
				fv.pos = verticesG.get(vertexIndex - 1);
			} else {
				// log.log(SEVERE, "Index for geometric vertex=" + vertexIndex +
			...
			if (vertexIndex != EMPTY_VERTEX_VALUE) {
				...
				if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesT.size())) {
					// Note: vertex indices are 1-indexed, i.e. they start at
					// one, so we offset by -1 for the 0-indexed array lists.
					fv.tex = verticesT.get(vertexIndex - 1);
				} else {
					// log.log(SEVERE, "Index for texture vertex=" + vertexIndex
					// + " is out of the current range of texture vertex values
					// 1 to " + verticesT.size() + ", ignoring");
				}
			}
			...
			if (vertexIndex != EMPTY_VERTEX_VALUE) {
				...
				if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesN.size())) {
					...
					// + " is out of the current range of vertex normal values 1
					// to " + verticesN.size() + ", ignoring");
				}
			}

			if (fv.pos == null) {
				// log.log(SEVERE, "Can't add vertex to face with missing
				// vertex! Throwing away face.");
				faceErrorCount++;
				return;
			}
	...
	public void addMapLib(String[] names) {
		if (null == names) {
			// log.log(INFO, "@TODO: ERROR! Got a maplib line with null names
			// array - blank group line? (i.e. \"g\\n\" ?)");
			return;
		}
		if (names.length == 1) {
			// log.log(INFO, "@TODO: Got a maplib line with one name=|" +
			// names[0] + "|");
			return;
		}
	...
	public void doneParsingObj(String filename) {
		// log.log(INFO, "Loaded filename '" + filename + "' with " +
		// verticesG.size() + " verticesG, " + verticesT.size() + " verticesT, "
		// + verticesN.size() + " verticesN and " + faces.size() + " faces, of
		// which " + faceTriCount + " triangles, " + faceQuadCount + " quads,
		// and " + facePolyCount + " with more than 4 points, and faces with
		// errors " + faceErrorCount);
	}
}
